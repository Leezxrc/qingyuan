...
class AgentCore:
    ...
    def _auto_finish_coding_session(self, plan, tool_results, available):
        """
        当模型已经给出最终文字、却忘记调用 code_finish_session 时，
        由执行器补上这个机械收尾动作。

        这不会绕过验证：
        - 只读会话会直接安全结束并收回 Permit；
        - 写入会话若最新 revision 未验证，SystemTools 仍会返回
          CODING_SESSION_NOT_VERIFIED，执行器随后要求模型继续检查。
        """
        if getattr(plan, "verify_mode", "") != "coding_result":
            return None

        names = [name for name, _ in tool_results]
        if "code_begin_session" not in names:
            return None
        ...
        if finish_results and "CODING_SESSION_FINISHED" in finish_results[-1]:
            return None

        fn = available.get("code_finish_session")
        if fn is None:
            return None
        ...
        self.messages.append({
            "role": "tool",
            "tool_name": "code_finish_session",
            "content": result,
        })
        tool_results.append(("code_finish_session", result))
        return result
    ...
    def _process(self, user_input):
        ...
        while True:
            ...
            if not tool_calls:
                ...
                if action_intent:
                    auto_finish_result = self._auto_finish_coding_session(
                        plan,
                        tool_results,
                        available,
                    )
                    ...
                    if not verification.success:
                        # 写入任务如果只是“还没验证”，这是可恢复状态，
                        # 不把内部工具名通过 TTS 念出来，也不立刻结束任务。
                        if (
                            auto_finish_result
                            and "CODING_SESSION_NOT_VERIFIED" in auto_finish_result
                        ):
                            guard_loops += 1
                            if guard_loops > DESKTOP_TASK_MAX_GUARD_LOOPS:
                                print(
                                    "\n[Coding Agent] 最新代码仍未通过验证，本次任务停止。"
                                )
                                break

                            print(
                                "\n[Coding Agent] 最新代码尚未验证，正在继续运行必要检查……"
                            )
                            guard_note = (
                                "\n【Coding Agent 执行器】\n"
                                "你已经写过代码，但最新 revision 还没有通过真实检查。"
                                "\n不要向用户朗读内部工具名。"
                                "\n请立即运行适合当前修改类型的检查；通过后查看 diff，"
                                "再完成 Coding Session。"
                            )
                            continue
                ...
                if content.strip():
                    if self.runtime.voice_enabled:
                        # 屏幕继续保留完整回答；TTS 根据意图和真实工具结果
                        # 生成更短、更口语化的版本，避免逐字朗读代码/路径/diff。
                        #
                        # 兼容 Brain Backend 的旧/远程 Voice Proxy：
                        # 新接口 speak_response 可用时直接使用；
                        # 若运行时代理仍只有 speak，则本地生成口播摘要后降级调用 speak，
                        # 不允许因为代理接口版本差异让整轮对话崩溃。
                        speak_response = getattr(
                            self.voice,
                            "speak_response",
                            None,
                        )
                        if callable(speak_response):
                            speak_response(
                                content,
                                intent=intent,
                                tool_results=tool_results,
                            ...
                            if callable(speak) and str(spoken or "").strip():
                                if str(spoken or "").strip() != str(content or "").strip():
                                    print(
                                        "\n[口播摘要兼容模式] "
                                        + str(spoken)
                                    )
