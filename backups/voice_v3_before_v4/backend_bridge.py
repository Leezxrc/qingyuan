...
class RemoteVoiceProxy(VoiceService):
    """
...
class RemoteToolFactory:
    ...
    def code_git_status(self):
        """读取当前代码项目 Git 状态。"""
        return self._call(
            "code_git_status"
        )

    def code_git_diff(self):
        """读取本次代码会话触碰文件的 Git diff。"""
        return self._call(
            "code_git_diff"
        )
    ...
    def code_run_checks(
        ...
        """运行 compile / pytest / unittest 白名单检查。"""
        return self._call(
            "code_run_checks",
            check=check,
            target=target,
